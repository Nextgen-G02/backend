...
export const updatePurchase = async (req, res) => {
  try {
    ...
    // Revert previous financial effects of this specific purchase on the supplier credit
    // 1. Put back applied credit
    supplier.creditBalance += (purchase.appliedCredit || 0);
    
    // 2. Revert any credit generated by a previous overpayment on THIS purchase
    const previousRemainingCost = purchase.cost - (purchase.appliedCredit || 0);
    if (purchase.paidAmount > previousRemainingCost) {
        const previousExcess = purchase.paidAmount - previousRemainingCost;
        supplier.creditBalance -= previousExcess;
    }

    // Now apply the NEW payment logic
    ...
    let newActualPaid = parseFloat(paidAmount) || 0;
    let newBalance = 0;

    if (newActualPaid > currentRemainingCost) {
      // Overpayment becomes credit
      const excess = newActualPaid - currentRemainingCost;
      supplier.creditBalance += excess;
      newBalance = 0;
    } else {
      newBalance = currentRemainingCost - newActualPaid;
    }

    purchase.paidAmount = newActualPaid;
